<?php

class Vector3{

  public function __construct( $x = 0, $y = 0, $z = 0 ) {

    $this->x = $x;
    $this->y = $y;
    $this->z = $z;

  }




	function set ( $x, $y, $z ) {

    $this->x = $x;
    $this->y = $y;
    $this->z = $z;

    return $this;

  }

  function setScalar ( $scalar ) {

    $this->x = $scalar;
    $this->y = $scalar;
    $this->z = $scalar;

    return $this;

  }

		function setX ( $x ) {

			$this->x = $x;

			return $this;

		}

		function setY ( $y ) {

			$this->y = $y;

			return $this;

		}

		function setZ ( $z ) {

			$this->z = $z;

			return $this;

		}

		function setComponent( $index, $value ) {

			switch ( $index ) {

				case 0: $this->x = $value; break;
				case 1: $this->y = $value; break;
				case 2: $this->z = $value; break;
				default: throw new Error( '$index is out of range: ' + $index );

			}

			return $this;

		}

		function getComponent( $index ) {

			switch ( $index ) {

				case 0: return $this->x;
				case 1: return $this->y;
				case 2: return $this->z;
				default: throw new Error( '$index is out of range: ' + $index );

			}

		}

		function _clone() {

			return new Vector3( $this->x, $this->y, $this->z );

		}

		function copy( $v ) {

			$this->x = $v->x;
			$this->y = $v->y;
			$this->z = $v->z;

			return $this;

		}

		function add( $v, $w = null ) {

			if ( $w !== null ) {

//				console.warn( 'THREE.Vector3: .add() now only accepts one argument. Use .addVectors( a, b ) instead.' );
				return $this->addVectors( $v, $w );

			}

			$this->x += $v->x;
			$this->y += $v->y;
			$this->z += $v->z;

			return $this;

		}

		function addScalar ( $s ) {

			$this->x += $s;
			$this->y += $s;
			$this->z += $s;

			return $this;

		}

		function addVectors( $a, $b ) {

			$this->x = $a->x + $b->x;
			$this->y = $a->y + $b->y;
			$this->z = $a->z + $b->z;

			return $this;

		}

		function addScaledVector( $v, $s ) {

			$this->x += $v->x * $s;
			$this->y += $v->y * $s;
			$this->z += $v->z * $s;

			return $this;

		}

		function sub( $v, $w = null ) {

			if ( $w !== null ) {

//				console.warn( 'THREE.Vector3: .sub() now only accepts one argument. Use .subVectors( a, b ) instead.' );
				return $this->subVectors( $v, $w );

			}

			$this->x -= $v->x;
			$this->y -= $v->y;
			$this->z -= $v->z;

			return $this;

		}

		function subScalar( $s ) {

			$this->x -= $s;
			$this->y -= $s;
			$this->z -= $s;

			return $this;

		}

		function subVectors( $a, $b ) {

			$this->x = $a->x - $b->x;
			$this->y = $a->y - $b->y;
			$this->z = $a->z - $b->z;

			return $this;

		}

		function multiply( $v, $w ) {

			if ( $w !== null ) {

				console.warn( 'THREE.Vector3: .multiply() now only accepts one argument. Use .multiplyVectors( a, b ) instead.' );
				return $this->multiplyVectors( $v, $w );

			}

			$this->x *= $v->x;
			$this->y *= $v->y;
			$this->z *= $v->z;

			return $this;

		}

		function multiplyScalar( $scalar ) {

			if ( is_finite( $scalar ) ) {

				$this->x *= $scalar;
				$this->y *= $scalar;
				$this->z *= $scalar;

			} else {

				$this->x = 0;
				$this->y = 0;
				$this->z = 0;

			}

			return $this;

		}

		function multiplyVectors( $a, $b ) {

			$this->x = $a->x * $b->x;
			$this->y = $a->y * $b->y;
			$this->z = $a->z * $b->z;

			return $this;

		}
//
//		function applyEuler() {
//
//			$quaternion;
//
//			return function applyEuler( $euler ) {
//
//				if ( ($euler && $euler->isEuler) === false ) {
//
//					console.error( 'THREE.Vector3: .applyEuler() now expects an Euler rotation rather than a Vector3 and order.' );
//
//				}
//
//				if ( $quaternion === null ) quaternion = new Quaternion();
//
//				return $this->applyQuaternion( quaternion.setFromEuler( euler ) );
//
//			};
//
//		}(),
//
//		applyAxisAngle: function () {
//
//			var quaternion;
//
//			return function applyAxisAngle( axis, angle ) {
//
//				if ( quaternion === undefined ) quaternion = new Quaternion();
//
//				return $this->applyQuaternion( quaternion.setFromAxisAngle( axis, angle ) );
//
//			};
//
//		}(),

		function applyMatrix3( $m ) {

			$x = $this->x;
      $y = $this->y;
      $z = $this->z;
			$e = $m->elements;

			$this->x = $e[ 0 ] * $x + $e[ 3 ] * $y + $e[ 6 ] * $z;
			$this->y = $e[ 1 ] * $x + $e[ 4 ] * $y + $e[ 7 ] * $z;
			$this->z = $e[ 2 ] * $x + $e[ 5 ] * $y + $e[ 8 ] * $z;

			return $this;

		}

		function applyMatrix4( $m ) {

			// input: THREE.Matrix4 affine matrix

			$x = $this->x;
      $y = $this->y;
      $z = $this->z;
			$e = $m->elements;

			$this->x = $e[ 0 ] * $x + $e[ 4 ] * $y + $e[ 8 ]  * $z + $e[ 12 ];
			$this->y = $e[ 1 ] * $x + $e[ 5 ] * $y + $e[ 9 ]  * $z + $e[ 13 ];
			$this->z = $e[ 2 ] * $x + $e[ 6 ] * $y + $e[ 10 ] * $z + $e[ 14 ];

			return $this;

		}

		function applyProjection( $m ) {

			// input: THREE.Matrix4 projection matrix

			$x = $this->x;
      $y = $this->y;
      $z = $this->z;
			$e = $m->elements;
			$d = 1 / ( $e[ 3 ] * $x + $e[ 7 ] * $y + $e[ 11 ] * $z + $e[ 15 ] ); // perspective divide

			$this->x = ( $e[ 0 ] * $x + $e[ 4 ] * $y + $e[ 8 ]  * $z + $e[ 12 ] ) * d;
			$this->y = ( $e[ 1 ] * $x + $e[ 5 ] * $y + $e[ 9 ]  * $z + $e[ 13 ] ) * d;
			$this->z = ( $e[ 2 ] * $x + $e[ 6 ] * $y + $e[ 10 ] * $z + $e[ 14 ] ) * d;

			return $this;

		}

		function applyQuaternion( $q ) {

			$x = $this->x;
      $y = $this->y;
      $z = $this->z;
			$qx = $q->x;
      $qy = $q->y;
      $qz = $q->z;
      $qw = $q->w;

			// calculate quat * vector

			$ix =  $qw * $x + $qy * $z - $qz * $y;
			$iy =  $qw * $y + $qz * $x - $qx * $z;
			$iz =  $qw * $z + $qx * $y - $qy * $x;
			$iw = - $qx * $x - $qy * $y - $qz * $z;

			// calculate result * inverse quat

			$this->x = $ix * $qw + $iw * - $qx + $iy * - $qz - $iz * - $qy;
			$this->y = $iy * $qw + $iw * - $qy + $iz * - $qx - $ix * - $qz;
			$this->z = $iz * $qw + $iw * - $qz + $ix * - $qy - $iy * - $qx;

			return $this;

		}

//		function project() {
//
//			var matrix;
//
//			return function project( camera ) {
//
//				if ( matrix === undefined ) matrix = new Matrix4();
//
//				matrix.multiplyMatrices( camera.projectionMatrix, matrix.getInverse( camera.matrixWorld ) );
//				return $this->applyProjection( matrix );
//
//			};
//
//		}(),

//		unproject: function () {
//
//			var matrix;
//
//			return function unproject( camera ) {
//
//				if ( matrix === undefined ) matrix = new Matrix4();
//
//				matrix.multiplyMatrices( camera.matrixWorld, matrix.getInverse( camera.projectionMatrix ) );
//				return $this->applyProjection( matrix );
//
//			};
//
//		}(),

		function transformDirection( $m ) {

			// input: THREE.Matrix4 affine matrix
			// vector interpreted as a direction

			$x = $this->x;
      $y = $this->y;
      $z = $this->z;
			$e = $m->elements;

			$this->x = $e[ 0 ] * $x + $e[ 4 ] * $y + $e[ 8 ]  * $z;
			$this->y = $e[ 1 ] * $x + $e[ 5 ] * $y + $e[ 9 ]  * $z;
			$this->z = $e[ 2 ] * $x + $e[ 6 ] * $y + $e[ 10 ] * $z;

			return $this->normalize();

		}

		function divide( $v ) {

			$this->x /= $v->x;
			$this->y /= $v->y;
			$this->z /= $v->z;

			return $this;

		}

		function divideScalar( $scalar ) {

			return $this->multiplyScalar( 1 / $scalar );

		}

		function min( $v ) {

			$this->x = min( $this->x, $v->x );
			$this->y = min( $this->y, $v->y );
			$this->z = min( $this->z, $v->z );

			return $this;

		}

		function max( $v ) {

			$this->x = max( $this->x, $v->x );
			$this->y = max( $this->y, $v->y );
			$this->z = max( $this->z, $v->z );

			return $this;

		}

		function clamp( $min, $max ) {

			// This function assumes min < max, if this assumption isn't true it will not operate correctly

			$this->x = max( $min->x, min( $max->x, $this->x ) );
			$this->y = max( $min->y, min( $max->y, $this->y ) );
			$this->z = max( $min->z, min( $max>z, $this->z ) );

			return $this;

		}

		function clampScalar( $minVal, $maxVal ) {

			$min;
      $max;

      if ( $min === null ) {

        $min = new Vector3();
        $max = new Vector3();

      }

      $min->set( $minVal, $minVal, $minVal );
      $max->set( $maxVal, $maxVal, $maxVal );

      return $this->clamp( $min, $max );

			}



		function clampLength ( $min, $max ) {

			$length = $this->length();

			return $this->multiplyScalar( max( $min, min( $max, $length ) ) / $length );

		}

		function floor() {

			$this->x = floor( $this->x );
			$this->y = floor( $this->y );
			$this->z = floor( $this->z );

			return $this;

		}

		function ceil() {

			$this->x = ceil( $this->x );
			$this->y = ceil( $this->y );
			$this->z = ceil( $this->z );

			return $this;

		}

		function round() {

			$this->x = round( $this->x );
			$this->y = round( $this->y );
			$this->z = round( $this->z );

			return $this;

		}

		function roundToZero() {

			$this->x = ( $this->x < 0 ) ? ceil( $this->x ) : floor( $this->x );
			$this->y = ( $this->y < 0 ) ? ceil( $this->y ) : floor( $this->y );
			$this->z = ( $this->z < 0 ) ? ceil( $this->z ) : floor( $this->z );

			return $this;

		}

		function negate() {

			$this->x = - $this->x;
			$this->y = - $this->y;
			$this->z = - $this->z;

			return $this;

		}

		function dot( $v ) {

			return $this->x * $v->x + $this->y * $v->y + $this->z * $v->z;

		}

		function lengthSq() {

			return $this->x * $this->x + $this->y * $this->y + $this->z * $this->z;

		}

		function length() {

			return sqrt( $this->x * $this->x + $this->y * $this->y + $this->z * $this->z );

		}

		function lengthManhattan() {

			return abs( $this->x ) + abs( $this->y ) + abs( $this->z );

		}

		function normalize() {

			return $this->divideScalar( $this->length() );

		}

		function setLength( $length ) {

			return $this->multiplyScalar( $length / $this->length() );

		}

		function lerp( $v, $alpha ) {

			$this->x += ( $v->x - $this->x ) * $alpha;
			$this->y += ( $v->y - $this->y ) * $alpha;
			$this->z += ( $v->z - $this->z ) * $alpha;

			return lerpthis;

		}

		function lerpVectors( $v1, $v2, $alpha ) {

			return $this->subVectors( $v2, $v1 ).multiplyScalar( $alpha ).add( $v1 );

		}

		function cross( $v, $w ) {

			if ( $w !== null ) {

//				console.warn( 'THREE.Vector3: .cross() now only accepts one argument. Use .crossVectors( a, b ) instead.' );
				return $this->crossVectors( $v, $w );

			}

			$x = $this->x;
      $y = $this->y;
      $z = $this->z;

			$this->x = $y * $v->z - $z * $v->y;
			$this->y = $z * $v->x - $x * $v->z;
			$this->z = $x * $v->y - $y * $v->x;

			return $this;
    }

		function crossVectors( $a, $b ) {

			$ax = $a->x;
      $ay = $a->y;
      $az = $a->z;
			$bx = $b->x;
      $by = $b->y;
      $bz = $b->z;

			$this->x = $ay * $bz - $az * $by;
			$this->y = $az * $bx - $ax * $bz;
			$this->z = $ax * $by - $ay * $bx;

			return $this;

		}

		function projectOnVector( $vector ) {

			$scalar = $vector->dot( $this ) / $vector->lengthSq();

			return $this->copy( $vector )->multiplyScalar( $scalar );

		}

		function projectOnPlane($planeNormal) {

				$v1 = new Vector3();

				$v1->copy( $this )->projectOnVector( $planeNormal );

				return $this->sub( $v1 );

		}

		function reflect() {

				$v1 = new Vector3();

				return $this->sub( $v1->copy( $normal )->multiplyScalar( 2 * $this->dot( $normal ) ) );

		}

		function angleTo( $v ) {

			$theta = $this->dot( $v ) / ( sqrt( $this->lengthSq() * $v->lengthSq() ) );

			// clamp, to handle numerical problems

			return acos( max( -1, min( 1, $theta ) ) );

		}

		function distanceTo( $v ) {

			return sqrt( $this->distanceToSquared( $v ) );

		}

		function distanceToSquared( $v ) {

			$dx = $this->x - $v->x;
      $dy = $this->y - $v->y;
      $dz = $this->z - $v->z;

			return $dx * $dx + $dy * $dy + $dz * $dz;

		}

		function distanceToManhattan( $v ) {

			return abs( $this->x - $v->x ) + abs( $this->y - $v->y ) + abs( $this->z - $v->z );

		}

		function setFromSpherical( $s ) {

			$sinPhiRadius = sin( $s->phi ) * $s->radius;

			$this->x = $sinPhiRadius * sin( $s->theta );
			$this->y = cos( $s->phi ) * $s->radius;
			$this->z = $sinPhiRadius * cos( $s->theta );

			return $this;

		}

		function setFromCylindrical( $c ) {

			$this->x = $c->radius * sin( $c->theta );
			$this->y = $c->y;
			$this->z = $c->radius * cos( $c->theta );

			return $this;

		}

		function setFromMatrixPosition( $m ) {

			return $this->setFromMatrixColumn( $m, 3 );

		}

		function setFromMatrixScale( $m ) {

			$sx = $this->setFromMatrixColumn( $m, 0 )->length();
			$sy = $this->setFromMatrixColumn( $m, 1 )->length();
			$sz = $this->setFromMatrixColumn( $m, 2 )->length();

			$this->x = $sx;
			$this->y = $sy;
			$this->z = $sz;

			return $this;

		}

		function setFromMatrixColumn( $m, $index ) {

			if ( is_nan( $m ) ) {

//				console.warn( 'THREE.Vector3: setFromMatrixColumn now expects ( matrix, $index ).' );
				$temp = $m;
				$m = $index;
				$index = $temp;

			}

			return $this->fromArray( $m->elements, $index * 4 );

		}

		function equals( $v ) {

			return ( ( $v->x === $this->x ) && ( $v->y === $this->y ) && ( $v->z === $this->z ) );

		}

		function fromArray( $array, $offset ) {

      if ( $offset === null ) {

        $offset = 0;
        
      }

			$this->x = $array[ $offset ];
			$this->y = $array[ $offset + 1 ];
			$this->z = $array[ $offset + 2 ];

			return $this;

		}

		function toArray( $array, $offset ) {

      if ( $array === null ) {$array = [];}
      if ( $offset === null ) {$offset = 0;}

			$array[ $offset ] = $this->x;
			$array[ $offset + 1 ] = $this->y;
			$array[ $offset + 2 ] = $this->z;

			return $array;

		}

		function fromAttribute( $attribute, $index, $offset ) {

			if ( $offset !== null ) {

//				console.warn( 'THREE.Vector3: offset has been removed from .fromAttribute().' );

			}

			$this->x = $attribute->getX( $index );
			$this->y = $attribute->getY( $index );
			$this->z = $attribute->getZ( $index );

			return $this;

		}


}
